...
Throughout the present work, it is assumed that if σ 1 [r] was originally undefined, it will be created as an account with no code or state and zero balance and nonce.
* 9. Execution Model
...
** Transaction Validation
   The given gasUsed must correspond faithfully to the transactions listed: B H u , the total gas used in the block, must be equal to the accumulated gas used according to the final transaction:
 B H g = `(R) u
** Reward Application
   The application of rewards to a block involves raising the balance of the accounts of the beneficiary address of the block and each ommer by a certain amount.
** State & Nonce Validation
   We may now define the function, Γ, that maps a block B to its initiation state:
(146)
And finally define Φ, the block transition function, which maps an incomplete block B to a complete block B 0
...
** Random Numbers
   Providing random numbers within a deterministic system is, naturally, an impossible task. However, we can approximate with pseudo-random numbers by utilising data which is generally unknowable at the time of transacting. Such data might include the block’s hash, the block’s timestamp and the block’s beneficiary address. In order to make it hard for malicious miner to control those values, one should use the BLOCKHASH operation in order to use hashes of the previous 256 blocks as pseudo-random numbers.
...
*** Web References
- aside from :: 除; 以外; 此外
- turn aside away from :: 离开
- aside  from :: 除……外; 除……以外
** preliminary [prɪ'lɪmɪn(ə)rɪ]
...
*** Web References
- Preliminary :: 初步; 初始的; 预赛
- preliminary treatment :: 预处理; 前处理; 预备处理
- preliminary adjustment :: 预调整; 初调; 初步调整
...
*** Web References
- Collision :: 碰撞; 冲突; 撞车
- Elastic collision :: 弹性碰撞; 弹性碰撞; 弹性撞击
- collision insurance :: 碰撞保险; 碰撞险; 碰垫保险
** malicious [mə'lɪʃəs]
...
*** Web References
- Trivial :: 平凡 (数学); 琐碎的; 微不足道的
- Trivial Pursuit :: 棋盘问答; 打破砂锅问到底; 追根究底
- trivial name :: 惯用名; 俗名; 种名
...
*** Web References
- Consolidation :: 巩固; 合并; 固结
- secondary consolidation :: 次固结; 二次固结; 次压密
- vertical consolidation :: 纵向合并; 纵的合并; 垂直合并
...
*** Web References
- eventually :: 最后; 终究; 终于
- Eventually results :: 最终处理结果
- Eventually reach :: 最终会到达
** traverse ['trævəs; trə'vɜːs]
...
*** Web References
- Traverse :: 枕木; 摇臂; 雪佛兰
- connecting traverse :: 附合导线; 吊钩梁; 连接横杆
- traverse point :: 导线点; 横动针片
** consolidation [kən,sɒlɪ'deɪʃən]
...
*** Web References
- recast :: 重铸; 重浇〔混凝土; 重作
- recast membrane :: 铸膜; 再铸膜
- recast factors :: 预报因子
** adhesion [əd'hiːʒ(ə)n]
...
*** Web References
- adhesion :: 附着力; 沾黏; 粘附
- Adhesion coefficient :: 附着系数; 粘着系数; 粘附系数
- adhesion test :: 黏着测试; 附着力试验; 粘着测试
